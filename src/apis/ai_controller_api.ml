...
let create_transformer ~ai_transform_create_options_t =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Ai_transform_create_options.to_yojson ai_transform_create_options_t in
...
let delete_all_transformer_mappings () =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/mappings" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let export_transformer_results ~export_transformer_options_t =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/results/export" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Export_transformer_options.to_yojson export_transformer_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Export_transformer_response.of_yojson) resp body
...
let get_export_transformer_results_job ~id =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/results/export/{id}" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "id"  id in
    Cohttp_lwt_unix.Client.call `POST uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Export_transformer_result_job_dto.of_yojson) resp body
...
let get_transformer_mappings ?ai_transform_id ?entity_id ?entity_type ?(page = 0l) ?(size = 20l) ?(sort = `ASC) () =
    ...
    let uri = Request.maybe_add_query_param uri "entityType" Enums.show_createdeliverabilitytestoptions_scope entity_type in
...
let get_transformer_results ?email_id ?sms_id ?attachment_id ?ai_transform_id ?ai_transform_mapping_id ?entity_id ?entity_type ?(page = 0l) ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
    ...
    let uri = Request.maybe_add_query_param uri "attachmentId" (fun x -> x) attachment_id in
    let uri = Request.maybe_add_query_param uri "aiTransformId"  ai_transform_id in
...
let get_transformer_results_table ~include_meta_data ~flatten_arrays_to_rows ?email_id ?sms_id ?attachment_id ?ai_transform_id ?ai_transform_mapping_id ?entity_id ?entity_type ?(page = 0l) ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/results/table" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.maybe_add_query_param uri "emailId"  email_id in
    let uri = Request.maybe_add_query_param uri "smsId"  sms_id in
    ...
    let uri = Request.add_query_param uri "size" Int32.to_string size in
    let uri = Request.add_query_param uri "sort" Enums.show_sortdirection sort in
    let uri = Request.add_query_param uri "includeMetaData" string_of_bool include_meta_data in
    let uri = Request.add_query_param uri "flattenArraysToRows" string_of_bool flatten_arrays_to_rows in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Page_table_data.of_yojson) resp body

let get_transformers ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?(_include = []) () =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let invoke_transformer ~invoke_transformer_options_t =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/invoke" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Invoke_transformer_options.to_yojson invoke_transformer_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Conditional_structured_content_result.of_yojson) resp body

let test_transformer_mapping_match ~id ?email_id ?sms_id ?attachment_id () =
    let open Lwt in
    let uri = Request.build_uri "/ai/transformer/mappings/{id}/match" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "id"  id in
    let uri = Request.maybe_add_query_param uri "emailId"  email_id in
    ...
    let uri = Request.maybe_add_query_param uri "attachmentId" (fun x -> x) attachment_id in
    Cohttp_lwt_unix.Client.call `POST uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Ai_transform_mapping_match_result.of_yojson) resp body

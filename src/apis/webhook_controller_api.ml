...
let get_all_webhook_results ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_webhook_results ~webhook_id ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in

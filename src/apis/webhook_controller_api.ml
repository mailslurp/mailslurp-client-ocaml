...
let get_all_webhooks ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?search_filter ?since ?inbox_id ?ai_transformer_id ?phone_id ?before ?health ?event_type ?url () =
    let open Lwt in
...
let get_inbox_webhooks_paginated ~inbox_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?search_filter ?since ?before ?health ?event_type () =
    let open Lwt in
...
let get_phone_number_webhooks_paginated ~phone_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?since ?before ?event_type ?search_filter ?health () =
    let open Lwt in

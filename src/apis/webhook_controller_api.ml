...
let create_webhook_for_ai_transformer ~transformer_id ~create_webhook_options_t =
    let open Lwt in
    let uri = Request.build_uri "/ai-transformers/{transformerId}/webhooks" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "transformerId"  transformer_id in
    let body = Request.write_as_json_body Create_webhook_options.to_yojson create_webhook_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Webhook_dto.of_yojson) resp body
...
let get_all_account_webhooks ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?since ?before ?event_type ?health ?search_filter () =
    let open Lwt in
    ...
    let uri = Request.maybe_add_query_param uri "health" Enums.show_healthstatus health in
...
let get_all_webhook_endpoints ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?search_filter ?since ?inbox_id ?phone_id ?before ?health ?event_type () =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/endpoints" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.add_query_param uri "page" Int32.to_string page in
    let uri = Request.add_query_param uri "size" Int32.to_string size in
    ...
    let uri = Request.maybe_add_query_param uri "eventType" Enums.show_eventname event_type in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Page_webhook_endpoint_projection.of_yojson) resp body

let get_all_webhook_results ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?search_filter ?since ?before ?unseen_only ?result_type ?event_name ?min_status_code ?max_status_code ?inbox_id ?sms_id ?attachment_id ?email_id ?phone_id ?ai_transformer_id () =
    let open Lwt in
    ...
    let uri = Request.maybe_add_query_param uri "resultType" Enums.show_resulttype result_type in
    let uri = Request.maybe_add_query_param uri "eventName" Enums.show_eventname event_name in
    let uri = Request.maybe_add_query_param uri "minStatusCode" Int32.to_string min_status_code in
    let uri = Request.maybe_add_query_param uri "maxStatusCode" Int32.to_string max_status_code in
    let uri = Request.maybe_add_query_param uri "inboxId"  inbox_id in
    ...
    let uri = Request.maybe_add_query_param uri "phoneId"  phone_id in
    let uri = Request.maybe_add_query_param uri "aiTransformerId"  ai_transformer_id in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Page_webhook_result.of_yojson) resp body

let get_all_webhooks ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?search_filter ?since ?inbox_id ?ai_transformer_id ?phone_id ?before ?health ?event_type ?url ?event_type_source ?include_account_wide () =
    let open Lwt in
    ...
    let uri = Request.maybe_add_query_param uri "eventTypeSource" Enums.show_eventtypesource event_type_source in
    let uri = Request.maybe_add_query_param uri "includeAccountWide" string_of_bool include_account_wide in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Page_webhook_projection.of_yojson) resp body

let get_inbox_webhooks_paginated ~inbox_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?search_filter ?since ?before ?health ?event_type ?include_account_wide () =
    let open Lwt in
...
let get_phone_number_webhooks_paginated ~phone_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?since ?before ?event_type ?search_filter ?health ?include_account_wide () =
    let open Lwt in
...
let get_test_webhook_payload_new_ai_transform_result () =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/test/new-ai-transform-result-payload" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Webhook_new_ai_transform_result_payload.of_yojson) resp body
...
let get_webhook_results ~webhook_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?search_filter ?since ?before ?unseen_only ?result_type ?event_name ?min_status_code ?max_status_code ?inbox_id ?sms_id ?attachment_id ?email_id ?phone_id ?ai_transformer_id () =
    let open Lwt in
...
let get_webhook_results_count ~webhook_id =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/{webhookId}/results/count" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "webhookId"  webhook_id in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
...
let get_webhooks ~inbox_id ?(page = 0l) ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
    let uri = Request.build_uri "/inboxes/{inboxId}/webhooks" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    ...
    let uri = Request.add_query_param uri "sort" Enums.show_sortdirection sort in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as_list_of (JsonSupport.unwrap Webhook_projection.of_yojson) resp body

let redrive_all_webhook_results () =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/results/redrive" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    Cohttp_lwt_unix.Client.call `POST uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Webhook_redrive_all_result.of_yojson) resp body
...
let update_webhook ~webhook_id ~create_webhook_options_t ?inbox_id ?ai_transformer_id ?phone_number_id ?override_auth () =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/{webhookId}" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    ...
    let uri = Request.maybe_add_query_param uri "overrideAuth" string_of_bool override_auth in
    let body = Request.write_as_json_body Create_webhook_options.to_yojson create_webhook_options_t in
...
let wait_for_webhook_results ~webhook_id ~expected_count ~timeout =
    let open Lwt in
    let uri = Request.build_uri "/webhooks/{webhookId}/wait" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "webhookId"  webhook_id in
    let uri = Request.add_query_param uri "expectedCount" Int32.to_string expected_count in
    let uri = Request.add_query_param uri "timeout" Int32.to_string timeout in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as_list_of (JsonSupport.unwrap Webhook_result_dto.of_yojson) resp body

...
let create_new_inbox_ruleset ~create_inbox_ruleset_options_t ?inbox_id () =
    let open Lwt in
...
let test_inbox_ruleset_receiving ~test_inbox_ruleset_receiving_options_t =
    let open Lwt in
    let uri = Request.build_uri "/rulesets/test-receiving" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Test_inbox_ruleset_receiving_options.to_yojson test_inbox_ruleset_receiving_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Test_inbox_ruleset_receiving_result.of_yojson) resp body

let test_inbox_ruleset_sending ~test_inbox_ruleset_sending_options_t =
    let open Lwt in
    ...
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Test_inbox_ruleset_sending_result.of_yojson) resp body

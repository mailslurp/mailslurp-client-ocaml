...
let wait_for_email_count ?count ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_latest_email ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_matching_emails ~match_options ?count ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_matching_first_email ~match_options ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_nth_email ?delay ?inbox_id ?(index = 0l) ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in

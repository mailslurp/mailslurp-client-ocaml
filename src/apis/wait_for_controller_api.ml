...
let wait_for_latest_sms ~wait_for_single_sms_options_t =
    let open Lwt in
    let uri = Request.build_uri "/waitForLatestSms" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Wait_for_single_sms_options.to_yojson wait_for_single_sms_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->

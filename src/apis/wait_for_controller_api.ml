...
let wait_for_email_count ?before ?count ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_latest_email ?before ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_matching_emails ~match_options ?before ?count ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_matching_first_email ~match_options ?before ?delay ?inbox_id ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in
...
let wait_for_nth_email ?before ?delay ?inbox_id ?(index = 0l) ?since ?sort ?timeout ?(unread_only = false) () =
    let open Lwt in

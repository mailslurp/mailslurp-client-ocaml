...
let add_contacts_to_group ~group_id ~update_group_contacts_option =
    let open Lwt in
    ...
    let body = Request.write_as_json_body Update_group_contacts.to_yojson update_group_contacts_option in
    Cohttp_lwt_unix.Client.call `PUT uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Group_contacts_dto.of_yojson) resp body

let create_group ~create_group_options =
    let open Lwt in
    let uri = Request.build_uri "/groups" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Create_group_options.to_yojson create_group_options in
...
let get_all_groups ?before ?(page = 0l) ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_group_with_contacts_paginated ~group_id ?before ?(page = 0l) ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let remove_contacts_from_group ~group_id ~update_group_contacts_option =
    let open Lwt in
    let uri = Request.build_uri "/groups/{groupId}/contacts" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in

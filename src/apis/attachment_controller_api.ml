...
let get_attachment ~attachment_id =
    ...
    let uri = Request.replace_path_param uri "attachmentId" (fun x -> x) attachment_id in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Attachment_entity.of_yojson) resp body
...
let get_attachments1 ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?file_name_filter ?since ?before () =
    let open Lwt in
    let uri = Request.build_uri "/attachments" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let upload_attachment_bytes ~request_body ?content_type ?filename () =
    let open Lwt in
    ...
    let body = Request.write_as_json_body (JsonSupport.of_list_of ) request_body in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as_list_of (JsonSupport.to_string) resp body

let upload_multipart_form ?content_type ?filename ?x_filename ~inline_object_t () =
    let open Lwt in

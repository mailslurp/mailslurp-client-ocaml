...
let extract_attachment_text ~attachment_id ~extract_attachment_text_options_t () =
    let open Lwt in
    let uri = Request.build_uri "/attachments/{attachmentId}/text" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "attachmentId" (fun x -> x) attachment_id in
    let body = Request.write_as_json_body Extract_attachment_text_options.to_yojson extract_attachment_text_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Extract_attachment_text_result.of_yojson) resp body
...
let get_attachments ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?file_name_filter ?since ?before ?inbox_id ?email_id ?sent_email_id ?(_include = []) () =
    let open Lwt in
    let uri = Request.build_uri "/attachments" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    ...
    let uri = Request.maybe_add_query_param uri "sentEmailId"  sent_email_id in
...
let upload_multipart_form ?content_id ?content_type ?filename ?content_type_header ?x_filename ?x_filename_raw ?x_filesize ~inline_object_2_t () =
    let open Lwt in

...
let upload_attachment_bytes ?content_type ?content_type2 ?content_id ?filename ?filename2 () =
    let open Lwt in
...
let upload_multipart_form ?content_id ?content_type ?filename ?x_filename ~inline_object_t () =
    let open Lwt in

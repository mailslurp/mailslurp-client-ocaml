...
let create_or_update_inbox_retention_policy_for_account ~create_inbox_retention_policy_for_account_options_t =
    let open Lwt in
    let uri = Request.build_uri "/user/inbox-retention-policies/account" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Create_inbox_retention_policy_for_account_options.to_yojson create_inbox_retention_policy_for_account_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Inbox_retention_policy_dto.of_yojson) resp body

let delete_inbox_retention_policy_for_account () =
    let open Lwt in
    let uri = Request.build_uri "/user/inbox-retention-policies/account" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let get_entity_events ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?since ?before ?inbox_id ?email_id ?phone_id ?sms_id ?attachment_id ?filter () =
    ...
    let uri = Request.maybe_add_query_param uri "filter" Enums.show_entityeventitemprojection_eventtype filter in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
...
let get_entity_favorites ?(page = 0l) ?(size = 20l) ?(sort = `DESC) ?since ?before ?filter () =
    ...
    let uri = Request.maybe_add_query_param uri "since" (fun x -> x) since in
    let uri = Request.maybe_add_query_param uri "before" (fun x -> x) before in
    let uri = Request.maybe_add_query_param uri "filter" Enums.show_entitytype filter in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Page_entity_favourite_items.of_yojson) resp body

let get_inbox_retention_policy_for_account () =
    let open Lwt in
    let uri = Request.build_uri "/user/inbox-retention-policies/account" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Inbox_retention_policy_optional_dto.of_yojson) resp body

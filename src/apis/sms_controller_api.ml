...
let get_all_sms_messages ?phone_number ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?since ?before ?search ?(favourite = false) ?(_include = []) () =
    let open Lwt in
    let uri = Request.build_uri "/sms" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let get_sms_codes ~sms_id ~extract_codes_options_t () =
    let open Lwt in
    let uri = Request.build_uri "/sms/{smsId}/codes" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "smsId"  sms_id in
...
let mark_all_sms_as_read ?(read = true) ?phone_number_id () =
    let open Lwt in
    let uri = Request.build_uri "/sms/read" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let mark_sms_as_read ~sms_id ?(read = true) () =
    let open Lwt in
    let uri = Request.build_uri "/sms/{smsId}/read" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.replace_path_param uri "smsId"  sms_id in
    let uri = Request.add_query_param uri "read" string_of_bool read in

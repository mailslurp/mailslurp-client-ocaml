...
let export_entities ~api_key ~export_type ~output_format ?created_earliest_time ?created_oldest_time ?exclude_previously_exported ?filter ?list_separator_token () =
    let open Lwt in
...
let get_export_link ~export_type ~export_options ?api_key () =
    let open Lwt in
    let uri = Request.build_uri "/export" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    ...
    let uri = Request.add_query_param uri "exportType" Enums.show_exporttype export_type in
    let body = Request.write_as_json_body Export_options.to_yojson export_options in

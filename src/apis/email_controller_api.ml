...
let get_emails_offset_paginated ?(inbox_id = []) ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?(unread_only = false) ?search_filter ?since ?before ?favourited ?sync_connectors ?plus_address_id () =
    let open Lwt in

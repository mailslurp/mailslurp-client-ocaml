...
let get_emails_paginated ?(inbox_id = []) ?(page = 0l) ?(search_filter = "false") ?(size = 20l) ?(sort = `ASC) ?(unread_only = false) () =
    let open Lwt in
    let uri = Request.build_uri "/emails" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let get_organization_emails_paginated ?(inbox_id = []) ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) ?(unread_only = false) () =
    let open Lwt in

...
let get_emails_paginated ?before ?(inbox_id = []) ?(page = 0l) ?(search_filter = "false") ?since ?(size = 20l) ?(sort = `ASC) ?(unread_only = false) () =
    let open Lwt in
    let uri = Request.build_uri "/emails" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    ...
    let uri = Request.add_query_param uri "searchFilter" (fun x -> x) search_filter in
...
let get_organization_emails_paginated ?before ?(inbox_id = []) ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) ?(unread_only = false) () =
    let open Lwt in
...
let send_email_source_optional ?inbox_id ?use_domain_pool ~send_email_options () =
    let open Lwt in
    let uri = Request.build_uri "/emails" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in

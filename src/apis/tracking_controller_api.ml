...
let create_tracking_pixel ~create_tracking_pixel_options_t =
    let open Lwt in
    let uri = Request.build_uri "/tracking/pixels" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Create_tracking_pixel_options.to_yojson create_tracking_pixel_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Tracking_pixel_dto.of_yojson) resp body

let get_all_tracking_pixels ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?search_filter ?since ?before () =
    let open Lwt in
    let uri = Request.build_uri "/tracking/pixels" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in

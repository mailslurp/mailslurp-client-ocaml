...
let get_all_sent_tracking_pixels ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_sent_email_tracking_pixels ~id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_sent_emails ?inbox_id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_sent_organization_emails ?inbox_id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in

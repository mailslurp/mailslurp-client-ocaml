...
let create_totp_device_for_base32_secret_key ~create_totp_device_base32_secret_key_options_t =
    let open Lwt in
    let uri = Request.build_uri "/mfa/totp/device/base32SecretKey" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Create_totp_device_base32_secret_key_options.to_yojson create_totp_device_base32_secret_key_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Totp_device_dto.of_yojson) resp body

let create_totp_device_for_custom ~create_totp_device_custom_options_t =
    let open Lwt in
    let uri = Request.build_uri "/mfa/totp/device/custom" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let body = Request.write_as_json_body Create_totp_device_custom_options.to_yojson create_totp_device_custom_options_t in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Totp_device_dto.of_yojson) resp body

...
let create_new_email_address ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox ?use_short_address ?domain_name ?domain_id ?prefix () =
    let open Lwt in
...
let create_random_inbox ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox ?use_short_address ?domain_name ?domain_id ?prefix () =
    let open Lwt in
...
let send_email_query ~_to ?sender_id ?body ?subject () =
    let open Lwt in
    let uri = Request.build_uri "/sendEmailQuery" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
    let uri = Request.maybe_add_query_param uri "senderId"  sender_id in
    let uri = Request.add_query_param uri "to" (fun x -> x) _to in

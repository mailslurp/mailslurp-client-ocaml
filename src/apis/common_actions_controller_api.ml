...
let create_new_email_address ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox ?use_short_address ?domain_name ?domain_id () =
    let open Lwt in
...
let create_random_inbox ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox ?use_short_address ?domain_name ?domain_id () =
    let open Lwt in

...
let create_new_email_address ?allow_team_access ?description ?email_address ?expires_at ?expires_in ?favourite ?inbox_type ?name ?(tags = []) ?use_domain_pool () =
    let open Lwt in
...
let create_new_email_address1 ?allow_team_access ?description ?email_address ?expires_at ?expires_in ?favourite ?inbox_type ?name ?(tags = []) ?use_domain_pool () =
    let open Lwt in

...
let create_new_email_address ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox () =
    let open Lwt in
...
let create_random_inbox ?allow_team_access ?use_domain_pool ?expires_at ?expires_in ?email_address ?inbox_type ?description ?name ?(tags = []) ?favourite ?virtual_inbox () =
    let open Lwt in

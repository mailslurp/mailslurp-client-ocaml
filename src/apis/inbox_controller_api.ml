...
let create_inbox ?email_address ?(tags = []) ?name ?description ?use_domain_pool ?favourite ?expires_at ?expires_in ?allow_team_access ?inbox_type ?virtual_inbox ?use_short_address ?domain_id ?domain_name () =
    let open Lwt in
...
let get_all_inboxes ?(page = 0l) ?(size = 20l) ?(sort = `ASC) ?(favourite = false) ?search ?tag ?team_access ?since ?before ?inbox_type ?domain_id () =
    let open Lwt in

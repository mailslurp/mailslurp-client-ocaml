...
let create_inbox_ruleset ~inbox_id ~create_inbox_ruleset_options =
    let open Lwt in
...
let get_all_inboxes ?before ?(favourite = false) ?(page = 0l) ?search ?since ?(size = 20l) ?(sort = `ASC) ?tag ?(team_access = false) () =
    let open Lwt in
...
let get_emails ~inbox_id ?before ?delay_timeout ?limit ?min_count ?retry_timeout ?since ?size ?sort ?unread_only () =
    let open Lwt in
...
let get_inbox_emails_paginated ~inbox_id ?before ?(page = 0l) ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_inbox_sent_emails ~inbox_id ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_inboxes ?before ?since ?(size = 100l) ?(sort = `ASC) () =
    let open Lwt in
    let uri = Request.build_uri "/inboxes" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let get_organization_inboxes ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let list_inbox_rulesets ~inbox_id ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
    let uri = Request.build_uri "/inboxes/{inboxId}/rulesets" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let list_inbox_tracking_pixels ~inbox_id ?before ?(page = 0l) ?search_filter ?since ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in

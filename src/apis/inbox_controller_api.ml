...
let create_inbox ?email_address ?(tags = []) ?name ?description ?use_domain_pool ?favourite ?expires_at ?expires_in ?allow_team_access ?inbox_type ?virtual_inbox ?use_short_address () =
    let open Lwt in

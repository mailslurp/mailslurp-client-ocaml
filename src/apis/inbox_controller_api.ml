...
let get_emails ~inbox_id ?delay_timeout ?limit ?min_count ?retry_timeout ?since ?size ?sort ?unread_only () =
    let open Lwt in
...
let get_inbox_sent_emails ~inbox_id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let get_organization_inboxes ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
...
let list_inbox_rulesets ~inbox_id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in
    let uri = Request.build_uri "/inboxes/{inboxId}/rulesets" in
    let headers = Request.default_headers in
    let headers = Cohttp.Header.add headers "x-api-key" Request.api_key in
...
let list_inbox_tracking_pixels ~inbox_id ?(page = 0l) ?search_filter ?(size = 20l) ?(sort = `ASC) () =
    let open Lwt in

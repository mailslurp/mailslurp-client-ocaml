(*
 * This file has been generated by the OCamlClientCodegen generator for openapi-generator.
 *
 * Generated by: https://openapi-generator.tech
 *
 * Schema Sms_match_option.t : Options for matching SMS messages in a phone number. Each match option object contains a `field`, `should` and `value` property. Together they form logical conditions such as `BODY` should `CONTAIN` value.
 *)

type t = {
    (* Fields of an SMS object that can be used to filter results *)
    field: Enums.smsmatchoption_field;
    (* How the value of the email field specified should be compared to the value given in the match options. *)
    should: Enums.should;
    (* The value you wish to compare with the value of the field specified using the `should` value passed. For example `BODY` should `CONTAIN` a value passed. *)
    value: string;
} [@@deriving yojson { strict = false }, show ];;

(** Options for matching SMS messages in a phone number. Each match option object contains a `field`, `should` and `value` property. Together they form logical conditions such as `BODY` should `CONTAIN` value. *)
let create (field : Enums.smsmatchoption_field) (should : Enums.should) (value : string) : t = {
    field = field;
    should = should;
    value = value;
}


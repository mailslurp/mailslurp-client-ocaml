(*
 * This file has been generated by the OCamlClientCodegen generator for openapi-generator.
 *
 * Generated by: https://openapi-generator.tech
 *
 * Schema Page_ai_transform_result_projection.t : Paginated AI Transform result entities. Page index starts at zero. Projection results may omit larger entity fields. For fetching a full entity use the projection ID with individual method calls.
 *)

type t = {
    content: Ai_transform_result_projection_dto.t list;
    pageable: Pageable_object.t option [@default None];
    columns: string list;
    total_pages: int32;
    total_elements: int64;
    last: bool option [@default None];
    size: int32 option [@default None];
    number: int32 option [@default None];
    number_of_elements: int32 option [@default None];
    sort: Sort_object.t option [@default None];
    first: bool option [@default None];
    empty: bool option [@default None];
} [@@deriving yojson { strict = false }, show ];;

(** Paginated AI Transform result entities. Page index starts at zero. Projection results may omit larger entity fields. For fetching a full entity use the projection ID with individual method calls. *)
let create (columns : string list) (total_pages : int32) (total_elements : int64) : t = {
    content = [];
    pageable = None;
    columns = columns;
    total_pages = total_pages;
    total_elements = total_elements;
    last = None;
    size = None;
    number = None;
    number_of_elements = None;
    sort = None;
    first = None;
    empty = None;
}

